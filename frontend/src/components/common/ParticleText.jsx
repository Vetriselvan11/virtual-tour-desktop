...
const resolveFontSize = (value, container, fontWeight, fontFamily) => {
  ...
  const size = parseFloat(window.getComputedStyle(probe).fontSize) || 88;
  probe.remove();
  return size;
};
...
const ParticleText = ({
  ...
  return (
    <div
      ...
      style={{ position: 'relative', display: 'block', width: '100%', height: '100%', overflow: 'hidden', touchAction: 'none', ...style }}
      aria-label={text}
    >
      <canvas
        ref={canvasRef}
